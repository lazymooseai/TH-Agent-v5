...
# ── REAALIAIKAISET KULTTUURITAPAHTUMAT (Helsinki API) ─────────────────────────

@st.cache_data(ttl=3600)
def get_culture_live_events():
    """Hakee kerralla kaikki tänään Helsingissä päättyvät tapahtumat."""
    live_tapahtumat = {}
    try:
        now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
        # Haetaan koko kuluvan vuorokauden tapahtumat rajapinnasta
        ...
        for e in data:
            ...
            try:
                ...
                # Tallennetaan vain tapahtumat, jotka päättyvät tulevaisuudessa ja tämän vuorokauden aikana
...
def yhdista_kulttuuridata(paikat):
    """Yhdistää staattiset paikat Helsinki API:n reaaliaikaiseen dataan."""
    ...
    for p in paikat:
        ...
        # Käydään läpi API:sta haetut tapahtumat ja verrataan hakusanoihin
        ...
        # Jos esitystä ei löydy tälle illalle API:sta
        if not loydetty_live:
            if hakusanat: # Jos paikalla on hakusanat, mutta API ei antanut tulosta
                p["lopetus_html"] = f"<span class='no-event'>⚪ Ei julkista esitystä tänään API:ssa.</span><br><span style='color:#777;'>Tyypillisesti: {p['lopetus']}</span>"
            else: # Esim. Yksityisklubit, joilla ei ole julkista APIa
...
def venue_card(p):
    # Päivitetty venue_card käyttämään uutta 'lopetus_html'-kenttää jos se on luotu
...
with tab1:
    # UUSI: Lisätty "hakusanat", jonka perusteella sovellus yhdistää API-datan näihin kohteisiin
    kulttuuri_paikat = [
        {
        ...
        },
        {
            ...
            "kap": "300 hlö",
            "hakusanat": [], # Ei julkista API-dataa yksityistilaisuuksista
        ...
        },
        {
            ...
            "kap": "200 hlö",
            "hakusanat": [], # Ei julkista API-dataa
    ...
    # Yhdistetään live data staattisiin paikkoihin
