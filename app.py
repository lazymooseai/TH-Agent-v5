    ...
    try:
        ...
        for juna in data:
            ...
            if aika_str and aika_obj:
                tulos.append({"train": nimi, "origin": lahto_kaupunki, "time": aika_str, "dt": aika_obj, "delay": viive if viive and viive > 0 else 0})
...
# ── REAALIAIKAISET KULTTUURITAPAHTUMAT (Helsinki API Kohdennettu) ─────────────

@st.cache_data(ttl=3600)
def get_venue_next_event(hakusanat):
    """Hakee yksittäisen tapahtumapaikan seuraavan esityksen kohdennetulla haulla."""
    if not hakusanat: return None
    now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
    
    # Käytetään ensimmäistä hakusanaa (esim. "kaupunginteatteri") tehokkaaseen API-hakuun
    haku = hakusanat[0]
    url = f"https://linkedevents.api.hel.fi/v1/event/?start=now&sort=end_time&text={haku}&include=location&page_size=15"
    ...
    try:
        ...
        for e in data:
            ...
            # Varmistetaan Pythonissa, että paikan nimi täsmää (eikä kyseessä ole vain tekstiosuma kuvauksessa)
            if not any(sana in paikka_nimi for sana in hakusanat):
                continue
            ...
            # API palauttaa vain käynnissä olevia tai tulevia tapahtumia (start=now)
            if end_dt > now:
                nimi = e.get("name", {}).get("fi", "Tuntematon esitys")
                return {"nimi": nimi, "dt": end_dt}
    except Exception:
        pass
    return None

def yhdista_kulttuuridata(paikat):
    """Yhdistää staattiset paikat Helsinki API:n kohdennettuun tulevaisuus-dataan."""
    now = datetime.datetime.now(ZoneInfo("Europe/Helsinki"))
    
    for p in paikat:
        hakusanat = p.get("hakusanat", [])
        next_event = get_venue_next_event(hakusanat)
        
        if next_event:
            dt = next_event["dt"]
            nimi = next_event["nimi"]
            aika_str = dt.strftime("%H:%M")
            
            if dt.date() == now.date():
                # Tapahtuma on tänään (Vihreä)
                p["lopetus_html"] = f"<span class='live-event'>🟢 TÄNÄÄN: {nimi} (päättyy klo {aika_str})</span>"
            elif dt.date() == now.date() + datetime.timedelta(days=1):
                # Tapahtuma on huomenna (Sininen)
                p["lopetus_html"] = f"<span style='color:#5bc0de;font-weight:bold;'>🔵 HUOMENNA: {nimi} (päättyy klo {aika_str})</span>"
            else:
                # Tapahtuma on myöhemmin (Harmaa päivämäärällä)
                pvm_str = dt.strftime("%d.%m.")
                p["lopetus_html"] = f"<span style='color:#aaa;font-weight:bold;'>⚪ SEURAAVA ({pvm_str}): {nimi} (päättyy klo {aika_str})</span>"
        else:
            if hakusanat:
                p["lopetus_html"] = f"<span class='no-event'>⚪ Ei tiedossa olevia esityksiä API:ssa.</span><br><span style='color:#777;'>Tyypillisesti: {p['lopetus']}</span>"
            else:
                p["lopetus_html"] = f"<span class='endtime'>⏱ Tyypillinen lopetus: {p['lopetus']}</span>"
                
    return paikat

# Muut avoimet live-tapahtumat (Tab 5 varten)
@st.cache_data(ttl=3600)
...
if lento_virhe:
    st.markdown(f"<div class='taksi-card'><div class='card-title'>Finavia API</div><span style='color:#ff9999;'>⚠ {lento_virhe}</span><br><br><a href='https://www.finavia.fi/fi/lentoasemat/helsinki-vantaa/lennot?tab=arr' class='taksi-link' target='_blank'>Finavia – Saapuvat lennot →</a></div>", unsafe_allow_html=True)
else:
    ...
    for lento in lennot:
        ...
        lento_html += f"<b>{lento['time']}</b>  ·  {lento['origin']} <span style='color:#aaa;'>({lento['flight']})</span><br>  └ <span class='{tyyppi_css}'>{lento['type']}</span> | {lento['status']}<br><br>"
...
def venue_html(paikat): return "".join(venue_card(p) for p in paikat)

with tab1:
    kulttuuri_paikat = [
        {
            "nimi": "Helsingin Kaupunginteatteri (HKT)", "kap": "947 hlö",
            "hakusanat": ["kaupunginteatteri", "hkt"],
            "huomio": "Paras yksittäinen kohde. Eläintarha.", "lopetus": "klo 21:30–22:30",
            "linkki": "https://hkt.fi/kalenteri/", "teksti": "HKT Kalenteri", "badge": "badge-red",
        },
        {
            "nimi": "Kansallisooppera ja -baletti", "kap": "1 700 hlö",
            "hakusanat": ["ooppera", "baletti"],
            "huomio": "Ooppera = parhaat kyydit. Töölönlahti.", "lopetus": "klo 21:00–23:00",
            "linkki": "https://oopperabaletti.fi/kalenteri/", "teksti": "Ooppera Ohjelmisto", "badge": "badge-yellow",
        },
        {
            "nimi": "Kansallisteatteri", "kap": "1 000 hlö",
            "hakusanat": ["kansallisteatteri"],
            "huomio": "Aseman vieressä, Rautatientori.", "lopetus": "klo 21:00–22:30",
            "linkki": "https://www.kansallisteatteri.fi/ohjelmisto/ohjelmistokalenteri", "teksti": "Kansallisteatteri Kalenteri", "badge": "badge-blue",
        },
        {
            "nimi": "Musiikkitalo", "kap": "1 704 hlö",
            "hakusanat": ["musiikkitalo"],
            "huomio": "Mannerheimintie.", "lopetus": "klo 21:00–22:30",
            "linkki": "https://musiikkitalo.fi/tapahtumat/", "teksti": "Musiikkitalo Tapahtumat", "badge": "badge-blue",
        },
        {
            "nimi": "Tanssin talo (Kaapelitehdas)", "kap": "1 000 hlö",
            "hakusanat": ["tanssin talo", "kaapelitehdas"],
            "huomio": "Modernin tanssin keskus. Erkko-sali 700 hlö, Pannuhalli 235 hlö.", "lopetus": "klo 21:00–22:30",
            "linkki": "https://www.tanssintalo.fi/ohjelma", "teksti": "Tanssin talo Kalenteri", "badge": "badge-blue",
        },
        {
            "nimi": "Helsingin Suomalainen Klubi", "kap": "300 hlö",
            "hakusanat": [],
            "huomio": "Yritysjohto – pitkät iltakyydit arki-iltaisin. Kamppi.", "lopetus": "klo 22:00–01:00",
            "linkki": "https://tapahtumat.klubi.fi/tapahtumat/", "teksti": "Klubi Tapahtumat", "badge": "badge-red",
        },
        {
            "nimi": "Svenska Klubben", "kap": "200 hlö",
            "hakusanat": [],
            "huomio": "Korkeaprofiilinen – erityisseuranta tapahtumailtaisin. Kruununhaka.", "lopetus": "klo 22:00–01:00",
            "linkki": "https://klubben.fi/start/program/", "teksti": "Svenska Klubben Ohjelma", "badge": "badge-red",
        },
        {
            "nimi": "Finlandia-talo", "kap": "1 700 hlö",
            "hakusanat": ["finlandia-talo", "pikku-finlandia"],
            "huomio": "Kongressit ja gaalat. Hyvä yrityspoistumat iltaisin.", "lopetus": "klo 21:00–23:00",
            "linkki": "https://finlandiatalo.fi/finlandia-talo/tulevat-tapahtumat/", "teksti": "Finlandia-talo Kalenteri", "badge": "badge-yellow",
        },
    ]
    kulttuuri_paikat_live = yhdista_kulttuuridata(kulttuuri_paikat)
    st.markdown(f"<div class='taksi-card'>{venue_html(kulttuuri_paikat_live)}</div>", unsafe_allow_html=True)

with tab2:
    urheilu_paikat = [
        {
            "nimi": "HIFK – Nordis (jääkiekko)", "kap": "8 200 hlö",
            "huomio": "Paras seura kyytien kannalta. Poistumapiikki ~2,5 h kiekon jälkeen.", "lopetus": "klo 21:30–23:00 (60+60+20 min + mahdolliset jatkoajat)",
            "linkki": "https://liiga.fi/fi/ohjelma?kausi=2025-2026&sarja=runkosarja&joukkue=hifk&kotiVieras=koti", "teksti": "HIFK Kotiottelut", "badge": "badge-red",
        },
        {
            "nimi": "Kiekko-Espoo – Metro Areena", "kap": "8 500 hlö",
            "huomio": "Suurin areena. Kyydit myös Espoosta Helsinkiin.", "lopetus": "klo 21:30–23:00",
            "linkki": "https://liiga.fi/fi/ohjelma?kausi=2025-2026&sarja=runkosarja&joukkue=k-espoo&kotiVieras=koti", "teksti": "Kiekko-Espoo Kotiottelut", "badge": "badge-yellow",
        },
        {
            "nimi": "Veikkaus Arena (Jokerit & Tapahtumat)", "kap": "15 000 hlö",
            "huomio": "Jokereiden kotiottelut ja suurtapahtumat. Valtava poistumapiikki.", "lopetus": "klo 21:00–22:30 (jääkiekko), 23:00 (konsertit)",
            "linkki": "https://www.veikkausarena.fi/fi/all-events", "teksti": "Veikkaus Arena Tapahtumat", "badge": "badge-red",
        },
        {
            "nimi": "Bolt Arena (HJK)", "kap": "10 770 hlö",
            "huomio": "Veikkausliiga ja HJK kotiottelut. Töölö. Huono sää nostaa kyytikerrointa.", "lopetus": "klo 21:00–22:30 (45+45+lisäaika)",
            "linkki": "https://www.hjk.fi/ottelut/miehet/", "teksti": "HJK Otteluohjelma", "badge": "badge-blue",
        },
        {
            "nimi": "Olympiastadion (jalkapallo / tapahtumat)", "kap": "50 000 hlö",
            "huomio": "Maaottelut, konsertit. Suuri poistumapiikki.", "lopetus": "klo 21:30–23:30 (konserteilla myöhemmin)",
            "linkki": "https://olympiastadion.fi/tapahtumat", "teksti": "Olympiastadion Tapahtumat", "badge": "badge-red",
        },
    ]
